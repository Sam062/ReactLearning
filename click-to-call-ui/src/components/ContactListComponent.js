...
export function ContactLists() {
        ...
        ContactListService.getAllContactLists().then(
            response => {
                Array.isArray(response.data) ?
                    setContactList(response.data) : alert('response not an array');
            }
        );
    }
    ...
    const handleEditClick = (event, contactList) => {
        ...
        const editFormValues = {
            ...
            listName: contactList.listName.trim(),
            listDesc: contactList.listDesc.trim()
        }
                ...
                <tbody>
                    {
                        contactList.length > 0 ?
                            contactList.map((list, index) => {
                                return editListId === list.contactListId ?
                                    <tr key={list.contactListId + Math.random()}>
                                        <EditContactListRow editFormData={editFormData} setEditListId={setEditListId} setNewListAdded={setNewListAdded}/>
                                    </tr>
                                    :
                                    <tr key={list.contactListId + Math.random()} onDoubleClick={(e) => handleEditClick(e, list)}>
                                        <td>{++index}</td>
                                        <td>
                                            <Link to={'/contactListData/' + `${list.contactListId}` + "/" + `${list.listName}`} className=""
...
function EditContactListRow({ editFormData, setEditListId, setNewListAdded }) {

    const formik = useFormik({
        initialValues: {
            contactListId: editFormData.contactListId,
            listName: editFormData.listName.trim(),
            listDesc: editFormData.listDesc.trim()
        },
        validationSchema: yup.object({
            listName: yup.string().trim().required("List name required.").min(4, "Name too short.").max(15, "Name too long."),
            listDesc: yup.string().trim().required("List description required.").min(4, "Description too short.").max(50, "Description too long.")
        ...
        <>
            <td hidden>
                <input type="text" onChange={formik.handleChange}
                    value={formik.values.contactListId} name="contactListId" className="form-control" />
            </td>
            <td colSpan={2}>
                <input type="text" onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.listName}
                    name="listName" className={(formik.touched.listName && formik.errors.listName || ((formik.values.listName.trim() === editFormData.listName) && formik.values.listDesc.trim() === editFormData.listDesc)) ? "form-control is-invalid" : "form-control is-valid"} placeholder="List Name(4-15 Characters)" />
            </td>
            <td colSpan={3}>
                <input type="text"
                    onBlur={formik.handleBlur} onChange={formik.handleChange} value={formik.values.listDesc}
                    name="listDesc" className={(formik.touched.listDesc && formik.errors.listDesc || ((formik.values.listName.trim() === editFormData.listName) && formik.values.listDesc.trim() === editFormData.listDesc)) ? "form-control is-invalid" : "form-control is-valid"} placeholder="List Description(4-50 Characters)" />
            </td>
            <td>
                <button className="btn text-success" onClick={formik.handleSubmit}
                    disabled={formik.errors.listName || formik.errors.listDesc || formik.values.listName.trim().length <= 0 || formik.values.listDesc.trim().length <= 0 || ((formik.values.listName.trim() === editFormData.listName) && formik.values.listDesc.trim() === editFormData.listDesc)}>
